...
var stripX = [], stripY = [], dY = [], stripFontSize = [];
...
function setFontStrip(i) {
    stripX[i] = Math.floor(Math.random() * (canvas.width * 0.9)) + canvas.width * 0.05;
    stripY[i] = -Math.floor(Math.random() * (canvas.height)) - 100;
    dY[i] = Math.floor(Math.random() * 7) + 3;
    stripFontSize[i] = Math.floor(Math.random() * 6) + 14;
}
...
function render() {
    ...
    for (var j = 0; j < stripCount; j++) {
        context.font = stripFontSize[j] + 'px Courier';
...
$(document).ready(function() {
    ...
    if (!isMobile) { // Render canvas only in desktop
